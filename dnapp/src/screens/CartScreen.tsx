...
export const SomethingBadHappened = () =>
    Alert.alert("Sorry", "Something Bad Happened. Try Again Later");
...
> {
    ...
    handleLinking = async () => {
        ...
        const { data } = await instamojoApi.get(
            "/payment-requests/" + this.state.requestId
        );

        if (data.payment_request.status !== "Completed") {
            Alert.alert(
                "Sorry",
                "Something Bad Happened at our end. If you have already paid for the order, it will be refunded between 24 hours."
            );
        } else {
            Alert.alert(
                "Success",
                "Your Order has been place it will be delivered to your place as soon as possible"
            );
    ...
    handlecod = async (amount: number) => {
        ...
        try {
            if (user) {
                const result = await dbapi.post(
                    "/orders/" + user.uid + ".json",
                    {
    ...
    handleOnlineOrder = async (amount: number) => {
        try {
            if (firebase.auth().currentUser) {
                ...
                const { data } = await instamojoApi.post(
                    "/payment-requests/",
                    query
                );
            ...
            <View style={styles.screen}>
                <View style={{ flex: 1 }}>
                    ...
                    >
                        ...
                        <DNRow
                            onPress={() => {
                                this.setState({ cod: !this.state.cod });
                            }}
                            leftIcon={
                                <Checkbox
                                    status={
                                        this.state.cod ? "checked" : "unchecked"
                                    }
                                    uncheckedColor={Colors.blueGrey900}
                                    onPress={() => {
                                        this.setState({ cod: !this.state.cod });
                                    }}
                                />
